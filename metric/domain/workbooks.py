...
from metric.domain.models import recommended_turns, required_variations, turn_plan_lines
from metric.shared.text import parse_path_str
from metric.domain.models import ADVERSARIAL_PERSONA, FUNCTIONAL_ORIGINS, ORIGIN_GRAPH, ORIGIN_PROBE, ScenarioRow, IntakeData, Scenario, Step, TurnMeta, canonical_origin
...
def read_scenarios(path: str, intake: IntakeData) -> List[Scenario]:
    """Reconstruct full Scenario objects from a graph or metadata workbook (same Scenario_Metadata
    / Turn_Metadata layout). If a Scenario_Text sheet is present (metadata workbooks), its LLM-authored
    ...
    with sheets.open_for_reading(path, "a scenario space workbook") as workbook:
        ...
        for row in sheets.read_rows(workbook["Turn_Metadata"]):
            # Positional rather than numbered: these rows are written in order, and a metadata workbook
        ...
        # below is: a metadata workbook written before the Name column existed has Description where Name
    ...
    # hand-counted indices had to be kept in step with the writer by eye, and a metadata workbook written
    ...
    for row in metadata:
        ...
        # Translated on the way in, so an origin an older metadata workbook spells differently reads as
        ...
        scenarios[-1].description = description
        scenarios[-1].turn_plan = turn_plan
    return scenarios
...
def write_space_metadata(path: str, intake: IntakeData, scenarios: List[Scenario]) -> None:
    """The metadata workbook: graph metadata plus the authored owner-facing text."""
    workbook = Workbook()
    workbook.remove(workbook.active)
