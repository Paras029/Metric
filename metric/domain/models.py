...
# The persona every probe is walked by. A probe is the tool's own question rather than the
# owner's, so it is never declared in an intake: defined once here, and read back by name.
...
VARIATIONS_BY_MATERIALITY = {"High": 3, "Medium": 2, "Low": 1}


def turn_plan_lines(scenario: "Scenario") -> List[str]:
    """The turn plan as one instruction per turn, numbering stripped. An unnumbered plan reads as
    a single turn."""
...
def required_variations(materiality: str, mapping: dict = None) -> int:
    """Variations requested per scenario for a materiality tier."""
