...
def migrate(rows: List[List[str]], origin: str,
    ...
    user = prompts.render(_MIGRATE_PROMPT, origin=origin, sample=sample(rows))

    try:
        reply = call(complete, prompts.load(_SYSTEM_PROMPT), user,
...
def _speaker_values(answer: dict) -> Dict[str, str]:
    ...
    """
    from metric.phases.coverage.coverage.conversations import AGENT, USER
