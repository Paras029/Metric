...
class ScenarioWriter:
    ...
    def _repair(self, scenarios: List[Scenario], intake: IntakeData) -> int:
        ...
        rendered = [self._render_repair(chunk, intake) for chunk in pending]
        self._progress(f"Rewriting {len(broken)} scenarios", 0, len(broken))
        replies = call_batch(self._complete, system, rendered,
                             tier=config.stage_tier("WRITER", config.STANDARD),
                             max_concurrency=config.stage_concurrency("WRITER"),
                             cancel=self._cancel,
    ...
    def _render_repair(self, chunk: List[tuple], intake: IntakeData) -> str:
        ...
        return prompts.render(
            ...
            house_style=prompts.load("shared.house_style"),
