...
from metric.phases.intake.intake.evidence import FACETS, KIND_IMAGE, Claim, DocumentRef, EvidenceRecord, FacetAnswer, SourceRef
from metric.phases.intake.intake.grounding import Source
from metric.llm import cancellation, config, council, prompts
from metric.llm.gateway import ask_llm, ask_llm_with_images
from metric.shared import parse_json_object
from metric.shared.replies import prose
from metric.phases.intake.intake.context_document import FACET_HEADINGS, FACET_QUESTIONS
from metric.phases.intake.intake import diagram_structure
from metric.phases.intake.intake.readers import UnreadableDocument, is_image, load_image, read_document
...
class DocumentExtractor:
    ...
    def _read_only_diagram(self, entry: Tuple[Path, Tuple[str, bytes]],
        ...
        try:
            reply = parse_json_object(council.deliberate(
                self._describe_images, prompts.load(_SYSTEM_PROMPT), user,
    ...
    def _read_one_diagram(self, path: Path, image: Tuple[str, bytes], index: int,
        ...
        user = prompts.render(_DIAGRAM_READ_PROMPT, filename=path.name,
    ...
    def _synthesize_diagrams(self, readings: List[Tuple[str, dict]],
        ...
        user = prompts.render(prompt, facets=_facet_guide(), vocabulary=_vocabulary(),
                                    document=names, readings=blocks)
        try:
            reply = parse_json_object(council.deliberate(
                self._complete, prompts.load(_SYSTEM_PROMPT), user,
